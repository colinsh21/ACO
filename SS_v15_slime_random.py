...
#        self.w=this_ant/(total_ants-1.0)        
        ...
        #Set permuation
        self.set_perm_sos(self.sos,self.score,1,ants) 
        
        #sos_max,score_max=self.s_max(self.sos)
        #Set permuation
        self.set_perm_sos(self.sos,self.score,2,ants)           
...
#            print sys,key,col,t,dec
    ...
    def sys_structure(self,sys):
        ...
        last_nodes={} 
        for n in self.model.spatial.nodes(): #initialize on all nodes
            last_nodes[n]=[n]
        ...
        while not terminate:
            ...
            '''
            #terminate=1 #end after one step           
...
#                                    (min_frac_dist+1.0) # was .1
            ...
            sinks_included=1 #check if all sinks have path to a source
            for sink in t:
                    not_conn=not any([nx.has_path(s_sol,source,sink) for source in s])
                    if not_conn:
                        sinks_included=0
                        break
            if sinks_included==1:
                ...
                terminate=0
                #only propagate from starting nodes
                last_nodes={} 
                for n in self.model.spatial.nodes(): #initialize on all nodes
                    last_nodes[n]=[n]
                    
                #expand from last step
...
class Space(object):
    ...
    def rank_generation(self):
        ...
        #Equality of survivability
        #equality_sequence=[1,0] #[>=,<=]
        
        #Equality of vulnerability
        equality_sequence=[0,0]
    ...
    def ph_update(self,p_id,p_bounds):
        ...
        #fitness for a solution is 1/R(s), 
        # where R(s) is the solutions rank based on min ordering
        #s=[damage,cost]
        p_front=[]
        num_updating=len(p_id)
        #get total fitness
        f_tot=sum(1.0/d for d in range(1, num_updating + 1)) #total fitness by rank
        for ant in p_id:
            s=self.ant_by_id(ant).score
            p_front.append(s)

        temp_list_rank=map(list,zip(*p_front)) #two lists, one for each objective
        ...
        for ant in p_id:
            ...
            f=[1.0/r[0],1.0/r[1]]
            inc=[f[0]/f_tot,f[1]/f_tot] #maintain hypercube
...
#                    self.p[sys][n][col][1]['b'][b]+=add[1]
    ...
    def vizualize_sos(self,ant_id):
        ...
        plt.suptitle(r'Ant {}: Functionality Loss$={:1.2f}$, Capacity$={:6.2f}$'.format(ant_id,s[0],s[1]),
                  fontsize=17)
...
#            gen_num+=1
        ...
        ax2.set_xlabel(r'Functionality Loss, $\bar{\phi}$', size=13)
...
#                y_p.append(p[1])
        ...
        ax1.set_xlabel(r'Functionality Loss, $\bar{\phi}$', size=13)
